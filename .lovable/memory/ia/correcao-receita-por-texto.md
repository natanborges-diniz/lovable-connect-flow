...
- **Natan (15/05/2026)**: receita completa digitada **na 1ª mensagem** sem pedido prévio da IA era ignorada e LLM pedia foto. Corrigido com `hasStrongRxSignal` (~linha 4309): basta ambos os olhos terem **esfera** definida (parser por bloco só preenche OD+OE quando há rótulos explícitos dos dois). Receita só-esférica (sem cilindro/eixo/adição) é comum e válida — não exigimos cilindro nem adição. Padrões isolados (`od -2.50`) continuam sendo ignorados porque só preenchem 1 olho.
