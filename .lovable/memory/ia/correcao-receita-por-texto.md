...
- **Natan (15/05/2026)** + **monocular**: receita digitada **na 1ª mensagem** sem pedido prévio era ignorada. `hasStrongRxSignal` (~linha 4311) agora exige (a) rótulo explícito `OD|OE|OS` no texto E (b) esfera em **≥1 olho**. Aceita receita monocular (visão de 1 olho só é vendável) e receita só-esférica. Padrões isolados sem rótulo (`-2.50`) continuam bloqueados. **Confirmação é obrigatória** — `confirmed_by_client_at=null` força gate determinístico antes de qualquer cotação. Princípio: aceitar a possibilidade, sempre confirmar, nunca agir por conta. No gate de confirmação, `detectRxConfirmation` também passou a aceitar respostas literais como **"está certo" / "está certinho"** (além de `tá certo`), evitando loop de repetição da mesma pergunta.
