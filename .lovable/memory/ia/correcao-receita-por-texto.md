...
- **Natan (15/05/2026)**: receita completa digitada **na 1ª mensagem** sem pedido prévio da IA (`Quero orçamento de multifocal. OD -2,00 -0,50 180 OE -1,75 -0,75 170 Adição +2,00`) era ignorada (`[RX-FIRST-TYPED] Skipped: no prior request from IA`) e LLM pedia foto da receita. Corrigido com `hasStrongRxSignal` (~linha 4299): se ambos os olhos têm esfera + (cilindro OU eixo em algum) ou adição, aceita como `client_typed_first` mesmo sem pedido prévio. Padrões isolados (`od -2.50`) continuam sendo ignorados.
