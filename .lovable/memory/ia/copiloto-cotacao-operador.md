...
- **NL → filtros:** quando `query_natural` vem preenchido, chama Lovable AI Gateway (`google/gemini-3-flash-preview`, temp 0) só para extrair `{ preferencia_marca, filtro_blue, filtro_photo, material_policarbonato, descarte, is_toric, preco_max }`. Filtros explícitos do form sobrepõem o NL. `preferencia_marca` é texto livre (marca + família + tratamento + material).
- **Busca por termo (ago/2026):** `applyTokenSearch` quebra o termo em palavras e exige TODAS (AND entre tokens), cada token casando em `brand|family|treatment|index_name` (óculos/catálogo) ou `fornecedor|produto` (LC). Materiais Essilor foram normalizados no banco: `S156/S167/S174 → Stylis 1.56/1.67/1.74`, `Orma → Orma 1.50`, `Airwear → Airwear 1.59`, `Poli → Policarbonato 1.59` — por isso "Varilux Stylis" agora encontra. Códigos opacos da DMax (`1.56_A56`, `1.67_F67`…) seguem pendentes de normalização.
