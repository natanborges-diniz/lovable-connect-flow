...
-- 1) Carência D+5: data a partir da qual o crédito pode ser usado
ALTER TABLE public.cashback_credito
  ADD COLUMN IF NOT EXISTS liberado_em date;

-- Backfill: créditos existentes ficam liberados na data de geração (ou hoje se nulo)
UPDATE public.cashback_credito
   SET liberado_em = COALESCE(data_geracao, CURRENT_DATE)
 WHERE liberado_em IS NULL;

CREATE INDEX IF NOT EXISTS idx_cashback_credito_usavel
  ON public.cashback_credito (contato_id, liberado_em, data_expiracao)
  WHERE status = 'ativo' AND saldo > 0;

-- 2) Função atômica de resgate (security definer)
CREATE OR REPLACE FUNCTION public.cashback_registrar_resgate(
  _contato_id          uuid,
  _numero_venda        text,
  _valor_informado     numeric,
  _cashback_usado      numeric,
  _cod_empresa         text,
  _usuario_lancamento  uuid DEFAULT NULL
) RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  END IF;
  IF _numero_venda IS NULL OR length(trim(_numero_venda)) = 0 THEN
    RAISE EXCEPTION 'numero_venda_invalido';
  END IF;
  IF _valor_informado IS NULL OR _valor_informado <= 0 THEN
    RAISE EXCEPTION 'valor_invalido';
  END IF;
  IF _cashback_usado IS NULL OR _cashback_usado < 0 THEN
    RAISE EXCEPTION 'cashback_usado_invalido';
  END IF;

  -- (a) Config
  SELECT percentual, validade_dias, fator_resgate
    INTO _cfg
    FROM public.cashback_config
    ORDER BY atualizado_em DESC
    LIMIT 1;
  IF _cfg IS NULL THEN
    RAISE EXCEPTION 'cashback_config_ausente';
  END IF;
  ...
  -- (b) Validações de uso de cashback
  IF _cashback_usado > 0 THEN
    SELECT COALESCE(SUM(saldo), 0)
      INTO _saldo_disponivel
  ...
  -- (c) Consumo FIFO por data de expiração
  IF _cashback_usado > 0 THEN
    ...
    LOOP
      ...
      INSERT INTO public.cashback_resgate
        (credito_id, contato_id, valor_usado, numero_venda_uso)
    ...
    IF _restante > 0 THEN
      RAISE EXCEPTION 'consumo_incompleto: restante=%', _restante;
    END IF;
  END IF;

  -- (d) Inscrição da venda nova (idempotente via regua_registrar_venda)
  ...
  -- Marca origem como resgate quando houve uso de cashback (e ainda não tinha sido marcada)
  IF _cashback_usado > 0 AND _inscricao_id IS NOT NULL THEN
    UPDATE public.regua_inscricao
     ...
     WHERE id = _inscricao_id
       AND COALESCE(origem, '') <> 'RESGATE_CASHBACK';
  END IF;

  -- (e) Crédito novo (provisório)
  ...
  IF _novo_credito_id IS NOT NULL THEN
    INSERT INTO public.eventos_crm (contato_id, tipo, descricao, referencia_tipo, referencia_id, metadata)
    VALUES (
      _contato_id, 'cashback_gerado',
      'Cashback provisório: R$ ' || _valor_gerado::text || ' (libera ' || to_char(_liberado_em, 'DD/MM') || ')',
      'cashback_credito', _novo_credito_id,
      jsonb_build_object(
        'numero_venda', _numero_venda,
        'valor_base', _valor_base,
        'valor_gerado', _valor_gerado,
        'percentual', _cfg.percentual,
        'liberado_em', _liberado_em,
        'data_expiracao', _expira_em,
        'inscricao_id', _inscricao_id
      )
    );
  END IF;

  -- (g) Saldo total atual após operação
  SELECT COALESCE(SUM(saldo), 0)
    INTO _saldo_total
    FROM public.cashback_credito
   WHERE contato_id = _contato_id
...
GRANT EXECUTE ON FUNCTION public.cashback_registrar_resgate(uuid, text, numeric, numeric, text, uuid)
