-- ═══════════════════════════════════════════════════════════════════
-- Funil v4 — transparência total por loja: adiciona futuros e
-- cancelados/reagendados (a linha soma o total de agendados) e TODAS
-- as taxas passam a usar os VENCIDOS como denominador (números reais:
-- o que já podia ter acontecido, excluindo visitas futuras).
-- Baseada na v3 — corrige a detecção de no-show/sem registro: os status
-- migram (no_show -> recuperacao -> abandonado), então o status atual
-- não serve como fonte. Fonte da verdade:
--   no-show real  = loja_confirmou_presenca IS FALSE (ou status no_show)
--   sem registro  = vencido + presenca NULL + status não-terminal
--                   (cancelado/reagendado ficam de fora; abandonado e
--                   recuperacao com presenca NULL CONTAM como sem registro)
-- Campos:
...
BEGIN
  ...
  SELECT g.id, g.contato_id, g.loja_nome, g.status, g.created_at, g.data_horario,
         ...
         -- No-show REAL: a loja registrou a ausência (o status depois migra
         -- para recuperacao/abandonado, então não serve como fonte)
         (g.loja_confirmou_presenca IS FALSE OR g.status = 'no_show') AS no_show_real,
         -- Sem registro: data já passou e a loja não marcou nada
         (g.data_horario < now()
          AND g.loja_confirmou_presenca IS NULL
          AND g.status NOT IN ('compareceu','atendido','orcamento','venda_fechada',
                               'no_show','cancelado','reagendado')) AS sem_registro,
         (g.status IN ('cancelado','reagendado')) AS cancel_reag,
  ...
  FROM (
    SELECT loja_nome AS loja,
           ...
           count(*) FILTER (WHERE NOT vencido) AS futuros,
           count(*) FILTER (WHERE cancel_reag) AS cancel_reag,
           count(*) FILTER (WHERE compareceu) AS compareceram,
           count(*) FILTER (WHERE no_show_real) AS no_show,
           count(*) FILTER (WHERE sem_registro) AS sem_registro,
           count(*) FILTER (WHERE no_show_real OR sem_registro) AS no_show_total,
           count(*) FILTER (WHERE venda) AS vendas,
           COALESCE(sum(valor_venda) FILTER (WHERE venda),0) AS valor_vendas,
           round(100.0 * count(*) FILTER (WHERE compareceu) / NULLIF(count(*) FILTER (WHERE vencido),0), 1) AS pct_comparecimento,
           round(100.0 * count(*) FILTER (WHERE no_show_real OR sem_registro)
  ...
  SELECT jsonb_build_object(
    ...
    'no_show', (SELECT count(*) FROM _ag WHERE no_show_real),
    'sem_registro', (SELECT count(*) FROM _ag WHERE sem_registro),
    'no_show_pct', (SELECT round(100.0 * count(*) FILTER (WHERE no_show_real) / NULLIF(count(*) FILTER (WHERE vencido),0),1) FROM _ag),
    'vencidos', (SELECT count(*) FROM _ag WHERE vencido),
    'futuros', (SELECT count(*) FROM _ag WHERE NOT vencido),
    'cancel_reag', (SELECT count(*) FROM _ag WHERE cancel_reag),
    'falta_total_pct', (SELECT round(100.0 * count(*) FILTER (WHERE no_show_real OR sem_registro)
