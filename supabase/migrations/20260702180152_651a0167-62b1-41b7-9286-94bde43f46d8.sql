...
BEGIN
  ...
  vendas_agg AS (
    SELECT
      ...
      (array_agg(b.cod_empresa ORDER BY b.criado_em DESC))[1] AS ultima_loja,
  ...
  SELECT
    ...
    COALESCE(c.saldo_a_vencer,0),
    COALESCE(c.saldo_vencido,0),
    COALESCE(c.saldo_utilizado,0),
    c.proxima_expiracao,
    v.ultima_venda_em,
    COALESCE(c.creditos,'[]'::jsonb),
    v.vendas
  FROM vendas_agg v
...
GRANT EXECUTE ON FUNCTION public.cashback_clientes_consolidado(text, text[], int) TO authenticated, service_role;
