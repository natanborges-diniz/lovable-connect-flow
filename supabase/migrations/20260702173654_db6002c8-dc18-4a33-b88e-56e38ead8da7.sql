...
CREATE OR REPLACE FUNCTION public.cashback_dashboard_kpis(
  _de date DEFAULT ((now() - '30 days'::interval))::date,
...
BEGIN
  ...
  -- Resolve nomes de loja -> códigos de empresa (via telefones_lojas)
...
-- RPC auxiliar para popular o filtro (nomes distintos de lojas ativas)
CREATE OR REPLACE FUNCTION public.cashback_listar_lojas()
RETURNS TABLE(nome_loja text)
LANGUAGE sql
SECURITY DEFINER
SET search_path TO 'public'
STABLE
AS $$
  SELECT DISTINCT tl.nome_loja
    FROM telefones_lojas tl
   WHERE tl.nome_loja IS NOT NULL
     AND tl.cod_empresa IS NOT NULL
   ORDER BY tl.nome_loja;
$$;

GRANT EXECUTE ON FUNCTION public.cashback_listar_lojas() TO authenticated;
GRANT EXECUTE ON FUNCTION public.cashback_dashboard_kpis(date, date, text[]) TO authenticated;
