-- Idempotência cashback_registrar_resgate: guard + unique index backstop
...
BEGIN
  ...
  -- (d') Resolve/cria inscrição ANTES do guard
  ...
  -- GUARD DE IDEMPOTÊNCIA: venda já processada?
  ...
  IF _novo_credito_id IS NOT NULL THEN
    INSERT INTO public.eventos_crm (contato_id, tipo, descricao, referencia_tipo, referencia_id, metadata)
    VALUES (_contato_id, 'cashback_gerado',
  ...
  RETURN jsonb_build_object(
    ...
    'credito_gerado', CASE WHEN _novo_credito_id IS NULL THEN NULL
      ELSE jsonb_build_object('id', _novo_credito_id, 'valor', _valor_gerado,
        'liberado_em', _liberado_em, 'data_expiracao', _expira_em)
    END,
    'resgate', _resgates,
    'inscricao_id', _inscricao_id,
    'ja_existia_inscricao', COALESCE((_venda_resp->>'ja_existia')::boolean, false)
...
-- Backstop: 1 crédito por inscrição
CREATE UNIQUE INDEX IF NOT EXISTS cashback_credito_inscricao_unico
  ON public.cashback_credito (inscricao_id)
