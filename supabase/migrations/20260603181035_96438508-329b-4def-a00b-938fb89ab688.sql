...
BEGIN
  ...
  -- 1) Atendente atribuído (respeitado mesmo se for loja — claim explícito)
  IF _atendente IS NOT NULL THEN
    RETURN ARRAY[_atendente];
  END IF;

  -- 2) Fallback por setor da coluna do contato — exclui usuários de loja
  ...
  -- 3) Fallback final: operadores corporativos (admin/colaborador) ativos
  SELECT ARRAY_AGG(DISTINCT p.id) INTO _ids
  FROM public.profiles p
  WHERE p.ativo = true
