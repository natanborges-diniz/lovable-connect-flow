...
BEGIN
  -- Tratamento robusto contra NULL: array vazio = sem escopo
  v_has_lojas   := COALESCE(array_length(NEW.lojas,1), 0)   > 0;
  v_has_setores := COALESCE(array_length(NEW.setores,1), 0) > 0;

  IF NEW.acesso_total THEN
    v_tipo := 'admin';
    v_role := 'admin';
  ELSE
    v_has_web := COALESCE((NEW.modulos ?| ARRAY['dashboard','crm','lojas','financeiro','ti','interno','estoque','tarefas','configuracoes']), false);
    v_has_loja_menu := COALESCE((NEW.modulos ? 'menu_loja'), false);

    -- Escopo por loja vence: lojas marcadas + sem setores = tipo loja
