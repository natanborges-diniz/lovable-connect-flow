-- ═══════════════════════════════════════════════════════════════════
-- Dashboard de Funil de Vendas — RPC agregadora + índices.
-- Funil (coorte de contatos com atendimento WhatsApp no período):
--   1 Contato inicial  → atendimentos canal='whatsapp'
--   2 Qualificado IA   → eventos receita/orçamento/CTA visita
--   3 Escalado humano  → eventos escalonamento_humano/escalar_consultor
--   4 Agendou          → agendamentos criados (dimensão de loja começa aqui)
--   5 Compareceu       → loja_confirmou_presenca ou status compatível
--   6 Venda fechada    → status venda_fechada (+ valor_venda)
-- Filtros: período, lojas (etapas 4-6 e quebras), fonte de lead (coorte).
-- Valores: vendas do funil (agendamentos.valor_venda) e faturamento
-- validado no período (regua_inscricao.valor_total_validado) como
-- reconciliação — coberturas diferentes, exibidos separadamente.
-- ═══════════════════════════════════════════════════════════════════

CREATE INDEX IF NOT EXISTS idx_eventos_crm_tipo_created
  ON public.eventos_crm (tipo, created_at);
CREATE INDEX IF NOT EXISTS idx_atendimentos_canal_created
  ON public.atendimentos (canal, created_at);
CREATE INDEX IF NOT EXISTS idx_agendamentos_loja_created
  ON public.agendamentos (loja_nome, created_at);
CREATE INDEX IF NOT EXISTS idx_agendamentos_status_created
  ON public.agendamentos (status, created_at);
...
DECLARE
  ...
  v_kpis jsonb;
BEGIN
  -- ── Coorte: contatos com atendimento WhatsApp iniciado no período ──
  ...
  -- ── Agendamentos do período (com filtro de loja) ligados à coorte ──
  CREATE TEMP TABLE _ag ON COMMIT DROP AS
  SELECT g.id, g.contato_id, g.loja_nome, g.status, g.created_at,
         ...
         (g.status IN ('no_show','abandonado','cancelado')) AS perdido,
  ...
  -- ── Funil (contatos distintos por etapa) ──
  ...
  -- ── Desempenho por loja (etapas 4-6) ──
  ...
  -- ── Por fonte de lead (coorte completa) ──
  ...
  -- ── KPIs + faturamento validado (regua) ──
