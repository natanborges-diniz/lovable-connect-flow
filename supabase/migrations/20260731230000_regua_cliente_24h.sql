-- ═══════════════════════════════════════════════════════════════════
-- Régua v2 — adiciona o toque ao CLIENTE via WhatsApp às 24h de vida:
-- um único template ("expira em 24 horas") com o link da página, que
-- também é o caminho de renovação quando expirar. Sem segunda mensagem
-- na expiração (redução de custo Meta); a retomada é por iniciativa do
-- cliente (página ou resposta no WhatsApp).
-- Mantém: lembrete à loja às 4h e expiração/arquivamento às 48h.
-- ═══════════════════════════════════════════════════════════════════

-- Alias do template (criado na Meta como cobranca_lembrete_cliente_v1)
...
DECLARE
  ...
  v_horas_lembrete constant int := 4;    -- loja
  v_horas_cliente  constant int := 24;   -- cliente (WhatsApp, uma vez)
...
BEGIN
  ...
  -- ── Fase 1: lembrete à loja (4h..48h, uma vez) ──
  FOR r IN
  ...
  -- ── Fase 2: lembrete ao CLIENTE via WhatsApp (24h..48h, uma vez) ──
  -- Dispara o template via pg_net → send-whatsapp-template (verify_jwt=false).
  FOR r IN
    SELECT * FROM solicitacoes s
    WHERE s.tipo IN ('link_pagamento','pix_pagamento')
      AND s.status NOT IN ('concluida','cancelada')
  ...
  LOOP
    ...
    -- Marca sempre (mesmo sem contato) para não reprocessar a cada hora
    UPDATE solicitacoes
  ...
  -- ── Fase 3: expiração automática (>48h) ──
  FOR r IN
    SELECT * FROM solicitacoes s
    WHERE s.tipo IN ('link_pagamento','pix_pagamento')
      AND s.status NOT IN ('concluida','cancelada')
