...
serve(async (req) => {
  ...
  try {
    ...
    if (isPixFluxo) {
      ...
      let obRes: Response;
      try {
        // OPTICAL_BUSINESS_URL contém caminho de function embutido; usa só o host.
        const OB_BASE = (OB_URL.match(/^https?:\/\/[^/]+/) || [OB_URL])[0];
        obRes = await fetch(`${OB_BASE}/functions/v1/pix-charges-v4`, {
          method: "POST",
          headers: { "Content-Type": "application/json", "x-service-key": OB_SECRET },
          body: JSON.stringify({
            action: "criar",
      ...
      if (rawTel.length >= 10 && nomeClienteRaw) {
        ...
        try {
          ...
          if (!tplRes.ok || tplJson?.status === "blocked_template_not_approved") {
            ...
            // ── v9: envia o copia-e-cola SOZINHO numa mensagem separada ──
            // Assim o cliente copia só o código (copiar a mensagem do template
            // levava o texto inteiro junto). Best-effort: exige janela de 24h
            // da Meta aberta (o send-whatsapp valida); sem janela, segue só o
            // template — o cliente ainda tem o código e o link na mensagem.
            try {
              const { data: atendCliente } = await supabase
                .from("atendimentos")
                .select("id")
                .eq("contato_id", contatoClienteId)
                .eq("canal", "whatsapp")
                .order("created_at", { ascending: false })
                .limit(1)
                .maybeSingle();
              if (atendCliente?.id && obData.pix_copia_cola) {
                const avulsoRes = await fetch(`${SUPABASE_URL}/functions/v1/send-whatsapp`, {
                  method: "POST",
                  headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${SERVICE}`,
                  ...
                  body: JSON.stringify({
                    atendimento_id: atendCliente.id,
                    texto: obData.pix_copia_cola,
                    remetente_nome: "Sistema Financeiro",
                  }),
                });
                console.log(
                  `[criar-solicitacao-loja] copia-cola avulso: ${avulsoRes.ok ? "enviado" : `nao_enviado_${avulsoRes.status}`}`,
                );
              }
            } catch (e) {
              console.warn("[criar-solicitacao-loja] copia-cola avulso falhou:", (e as Error).message);
            }
          } else {
            envioClienteStatus = "falhou";
            envioClienteErro = tplJson?.error || "resposta_inesperada";
