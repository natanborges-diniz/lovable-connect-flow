...
serve(async (req) => {
  ...
  try {
    ...
    if (isPixFluxo) {
      ...
      // OPTICAL_BUSINESS_URL historicamente inclui caminho de function embutido
      // (por isso TODAS as chamadas "pix-charges" caíam no payment-links via
      // roteamento de caminho extra do gateway). Aqui usamos só o host.
      ...
      const obGet = async (path: string) => {
        const r = await fetch(`${OB_BASE}/rest/v1/${path}`, { headers: obHeaders });
      ...
      try {
        ...
        const insRes = await fetch(`${OB_BASE}/rest/v1/payment_links`, {
          method: "POST",
        ...
        await fetch(`${OB_BASE}/rest/v1/payment_links?id=eq.${linkId}`, {
          method: "PATCH",
