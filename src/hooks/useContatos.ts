...
export function useUpdateContato() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async ({ id, ...updates }: { id: string; nome?: string; tipo?: TipoContato; estagio?: EstagioFunil; email?: string | null; telefone?: string | null; documento?: string | null; pipeline_coluna_id?: string }) => {
